...
from evaluation.test_cases import GOAL_ANALYZER_TEST_CASES

PROMPT_A = """You are a Fitness Goal Analyzer. Extract the user's goal,
timeframe, and constraints. Write a brief summary."""

PROMPT_B = """You are an expert Fitness Goal Analyzer. For every request:

1. Identify the PRIMARY goal (weight loss, muscle gain, endurance, etc.)
2. Extract the SPECIFIC target (kg, distance, weight, time)
3. Note the TIMEFRAME and required pace
4. Assess the user's FITNESS LEVEL (beginner/intermediate/advanced)
5. Capture TIME and EQUIPMENT constraints
6. Flag any unrealistic expectations

Output a clean structured profile with headers for each section.
Be concrete — use the user's exact numbers."""
...
def ab_test():
    ...
    for test in GOAL_ANALYZER_TEST_CASES:
        print(f"\n--- Test {test['id']} ---")
